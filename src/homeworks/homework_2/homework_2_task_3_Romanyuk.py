...
if __name__ == '__main__':
    read_file = input('file with commands [src].txt: ')
    write_file = input('output file [dst].txt: ')
    with open(write_file, 'w') as write:
        with open(read_file, 'r') as file:
            length_dna = int(file.readline())
            dna = string_to_node(file.readline().strip())
            command_number = int(file.readline())
